...
export async function getProducts(params = {}, locale = "nl") {
  try {
    ...
    const catsWithVisibility = allCatIds.length
      ? await fetch(
          wcUrl(
            "products/categories",
...
export async function getProductBySlug(slug, locale = "nl") {
  try {
    ...
    const categoriesWithVisibility = categoryIds.length
      ? await fetch(
          wcUrl(
            "products/categories",
    ...
    const res = await fetch(
      wcUrl(
        "products/categories",
        {
          slug,
          _fields:
            "id,name,slug,image,description,translations,meta_title,meta_description",
        },
        locale,
      ),
      { next: { revalidate: 3600 } },
