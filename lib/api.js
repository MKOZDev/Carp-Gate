...
export async function getProductBySlug(slug, locale = "nl") {
  try {
    ...
    product = applyVat(product);
    if (variations !== null) {
      product._variations = variations.map((v) => applyVat(v));
    }
  ...
  try {
    const url = wcUrl(
      "products",
      {
        ...
        per_page: perPage,
        status: "publish",
        _fields: "id,name,slug,images,price,regular_price,sale_price,on_sale",
      },
      locale,
    );
